...
import com.gumoxi.gumoximall.auth.feign.ThirdPartFeignService;
...
public class LoginController {
    ...
    public R sendCode(@RequestParam("phone") String phone) {
        // TODO 1、接口防刷
        String redisCode = stringRedisTemplate.opsForValue().get(AuthServerConstant.SMS_CODE_CACHE_PREFFIX + phone);
        ...
        // 2、验证码的再次校验 redis。存 key-phone ,value-code sms:code:13894890622 -> 2345
        String code = UUID.randomUUID().toString().substring(0, 5);
        String value = code + "_" + System.currentTimeMillis();
        // redis缓存验证码，防止同一个phone在60秒内再次发送验证码
        stringRedisTemplate.opsForValue().set(AuthServerConstant.SMS_CODE_CACHE_PREFFIX+phone, value, 10, TimeUnit.MINUTES);
        thirdPartFeignService.sendCode(phone, code);
        return R.ok();
    }

    /**
     * //TODO 重定向携带数据，有用session原理。将数据放入session中，
     *  只要跳转到下一个页面取出这个数据以后，session里面的数据就会删掉
     *  // TODO 分布式下的session问题  已使用SpringSession解决
...
//            model.addAttribute("errors", errors);
            ...
            //校验出错，转发到注册页面
            return "redirect:http://auth.gumoxi.com/reg.html";
        }
        // 真正注册，调用远程服务进行注册
        String code = vo.getCode();
        String s = stringRedisTemplate.opsForValue().get(AuthServerConstant.SMS_CODE_CACHE_PREFFIX + vo.getPhone());
        if(!StringUtils.isEmpty(s)) {
            if(code.equals(s.split("_")[0])) {
                // 删除验证码，令牌机制
                stringRedisTemplate.delete(AuthServerConstant.SMS_CODE_CACHE_PREFFIX + vo.getPhone());
